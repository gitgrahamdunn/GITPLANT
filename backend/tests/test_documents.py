...
def test_document_search_and_transmittal_flow():
    reset_db()
    ...
    assert search.json()["items"][0]["document_number"] == "MECH-5002"

    doc_id = second_doc.json()["id"]
    ...
    listed = client.get(f"/documents/{doc_id}/transmittals")
    assert listed.status_code == 200
    assert len(listed.json()) == 1
    assert listed.json()[0]["transmittal_number"] == "TRM-0001"
